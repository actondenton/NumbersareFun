// Number 1 Log Module
// Merged from: n1-log.js, n1-log-ticker-runtime.js
...
export function createLogTickerRuntime(deps) {
    ...
    function setAmbientMessage(entry) {
        ...
        const cat = normalizeLogCategory(entry.category);
        if (!isLogCategoryVisible(cat, getSettings().humorEnabled)) return;
    ...
    function renderMessageLogPageHtmlInternal(escapeHtml) {
        const visible = getVisibleLogEntries(actionLogEntries, getSettings().humorEnabled);
        if (visible.length === 0) {
            messageLogLastRenderedVisibleCount = 0;
            messageLogLastRenderedHeadSig = "";
            messageLogLastRenderedTailSig = "";
            return renderMessageLogPageHtml(visible, escapeHtml);
        }
    ...
    function refreshMessageLogPanelIfOpen(escapeHtml) {
        ...
        if (
            ...
            prevBody.insertAdjacentHTML(
                "beforeend",
                renderMessageLogLineHtml(visible[visible.length - 1], escapeHtml)
            );
    ...
    function startPeriodicAmbientAndAdaptive(runtimeOpts) {
        ...
        setInterval(() => {
            ...
            const source = LOG_MESSAGE_ENTRIES.filter(e => isLogCategoryVisible(e.category, getSettings().humorEnabled));
    ...
    return {
        ...
        renderMessageLogPageHtml: renderMessageLogPageHtmlInternal,
