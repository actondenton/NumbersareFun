import { ASCENSION_1_REQUIRED_TOTAL } from "./ascension.js";
import { NUMBER2_ASCENSION_READY_TOTAL } from "../number2/game.js";
...
export function createOverviewAscensionPanelsRefresh(deps) {
    let ascendButtonPointerGuardBound = false;

    function isAscendNumber1ButtonInteractionLocked(btn) {
        if (!btn) return false;
        if (btn.dataset.ascendPointerDown === "1") return true;
        try {
            if (btn.matches(":hover") || btn.matches(":active") || btn.matches(":focus-visible")) return true;
        } catch (_) {
            /* jsdom / older engines may lack :focus-visible */
        }
        return false;
    }

    function bindAscendButtonPointerGuard() {
        const pagePanelBodyEl = deps.getPagePanelBodyEl();
        if (!pagePanelBodyEl || ascendButtonPointerGuardBound) return;
        ascendButtonPointerGuardBound = true;
        function clearPointerDown(btn) {
            if (btn && btn.dataset) delete btn.dataset.ascendPointerDown;
        }
        pagePanelBodyEl.addEventListener(
            "pointerdown",
            function (e) {
                const btn = e.target.closest(".ascend-number-btn");
                if (btn) btn.dataset.ascendPointerDown = "1";
            },
            true
        );
        pagePanelBodyEl.addEventListener(
            "pointerup",
            function (e) {
                clearPointerDown(e.target.closest(".ascend-number-btn"));
            },
            true
        );
        pagePanelBodyEl.addEventListener(
    ...
    /** Update ascend-control copy without replacing the button node (keeps click target stable). */
    function patchNumber1AscendControlInPlace(control, digest) {
        const wrap = document.createElement("div");
        wrap.innerHTML = deps.renderNumber1AscendControlHtml(digest);
        const fresh = wrap.firstElementChild;
        if (!fresh || !control) return false;
        control.className = fresh.className;
        control.setAttribute("data-live-patch-digest", digest);
        const title = control.querySelector(".ascension-run-action__title");
        const status = control.querySelector(".ascension-run-action__status");
        const btn = control.querySelector(".ascension-run-action__btn");
        const freshTitle = fresh.querySelector(".ascension-run-action__title");
        const freshStatus = fresh.querySelector(".ascension-run-action__status");
        const freshBtn = fresh.querySelector(".ascension-run-action__btn");
        ...
        if (btn && freshBtn) {
            btn.disabled = freshBtn.disabled;
            if (freshBtn.getAttribute("aria-disabled") === "true") {
                btn.setAttribute("aria-disabled", "true");
            } else {
                btn.removeAttribute("aria-disabled");
            }
        }
        const banner = control.querySelector("[data-asc-run-time-banner]");
        const freshBanner = fresh.querySelector("[data-asc-run-time-banner]");
        if (freshBanner) {
            if (banner) {
                banner.outerHTML = freshBanner.outerHTML;
            } else {
                control.insertBefore(freshBanner.cloneNode(true), control.firstChild);
            }
        }
        return true;
    }
    ...
    function patchNumber1AscendControlIfOpen() {
        ...
        if (!control) return;
        const btn = control.querySelector(".ascension-run-action__btn");
        if (isAscendNumber1ButtonInteractionLocked(btn)) return;
        if (patchNumber1AscendControlInPlace(control, digest)) return;
        control.outerHTML = deps.renderNumber1AscendControlHtml(digest);
    }
            ...
            if (entry.number === 1) {
                ...
                if (cell) {
                    ...
                    if (cell.dataset.overviewAscSnap !== ascPart) {
                        const overviewAscBtn = cell.querySelector(".ascend-number-btn");
                        if (isAscendNumber1ButtonInteractionLocked(overviewAscBtn)) return;
