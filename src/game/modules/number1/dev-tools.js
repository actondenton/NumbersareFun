...
import { HandCounter, HAND_BASE_SPEED } from "./hands.js";
...
/** "1" = combos grant turbo meter points (default); "0" = off (isolate BH Ergosphere passive fill). */
const DEV_SESSION_COMBO_TURBO_FILL = "numbersarefun_dev_combo_turbo_fill";
...
export function attachN1DevTools(depsRaw) {
    ...
    function readDevComboTurboFillEnabledFromSession() {
        try {
            if (typeof sessionStorage === "undefined") return true;
            const v = sessionStorage.getItem(DEV_SESSION_COMBO_TURBO_FILL);
    ...
    function syncDevComboTurboFillFromSession() {
        const on = readDevComboTurboFillEnabledFromSession();
        if (deps.comboTurboFillFromCombosFlag) deps.comboTurboFillFromCombosFlag.set(on);
        const cb = els.devComboTurboFillCheckbox;
        if (cb) cb.checked = on;
    }
    ...
    if (els.devToolsToggle && els.devToolsPanel) {
        els.devToolsToggle.addEventListener("click", () => {
            ...
            if (show) {
                ...
                syncDevComboTurboFillFromSession();
    ...
    if (els.devComboTurboFillCheckbox) {
        els.devComboTurboFillCheckbox.addEventListener("change", () => {
            const on = !!els.devComboTurboFillCheckbox.checked;
            try {
                sessionStorage.setItem(DEV_SESSION_COMBO_TURBO_FILL, on ? "1" : "0");
            } catch (_) {}
            if (deps.comboTurboFillFromCombosFlag) deps.comboTurboFillFromCombosFlag.set(on);
            if (deps.addToLog) {
                deps.addToLog(
                    on
                        ? "Dev: combo → Turbo meter fill enabled."
                        : "Dev: combo → Turbo meter fill disabled (Ergosphere passive fill unchanged).",
                    "tip"
                );
            }
        });
