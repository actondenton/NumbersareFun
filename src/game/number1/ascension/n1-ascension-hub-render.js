import { BLACK_HOLE_PHASE1_ESSENCE_TARGET } from "../black-hole/number1-black-hole.js";
...
export function collectPurchasedAscensionGrantFlags(dep) {
...
/**
 * Hub header pills + grouped grant summary HTML (Phase 15a).
 *
 * @param {object} dep
 */
export function createAscensionHubRender(dep) {
    /**
     * Full, grouped summary of purchased ascension benefits for the hub header.
     * Uses `computeAscensionGrantTotals()` plus flags from owned nodes (near-miss, autobuy, turbo-scension unlocks).
     */
    function renderAscensionHubGrantsHtml() {
        if (!dep.getNumber1HasAscended()) return "";
        const esc = escapeAscensionHtml;
        const owned = dep.getNumber1AscensionNodeIds().length;
        if (!owned) {
            return "<p class=\"asc-hub-grants-empty\">No gems purchased yet. Unlock nodes on the map to see your benefits listed here.</p>";
        }
        const t = dep.computeAscensionGrantTotals();
        const gf = collectPurchasedAscensionGrantFlags(dep);
        const groups = [];
        ...
        const nr = dep.getNearMissToleranceRanks();
        ...
        const live = [];
        if (dep.getUnlockedHands() >= 2) {
            const cat = dep.getPatternCatalogMultiplier();
            const pat = dep.getAscensionComboPatternMult();
            live.push("Combo CPS right now (Combo Catalog \u00d7 Ascended Combo): \u00d7" + (cat * pat).toFixed(2) + " (needs 2+ hands)");
            live.push("Time Warp burst combo multiplier (with your stacks): \u00d7" + dep.getTimeWarpComboMultiplier().toFixed(2));
        } else {
            live.push("Unlock a second hand to enable combo CPS and live warp-stack multipliers.");
        }
        live.push("Turbo Boost count multiplier cap (this run): \u00d7" + (Math.round(dep.getTurboCountMultiplierMax() * 100) / 100).toLocaleString("en-US"));
        live.push("Turbo meter max (this run): " + dep.formatCount(dep.getTurboMeterMax()));
        live.push("Time Warp overflow ratio toward cap: " + (dep.getTimeWarpOverflowRatio() * 100).toFixed(0) + "% · max aura spawn span: \u2264" + dep.formatCount(Math.round(dep.getTimeWarpAuraSpawnSpanMaxSec())) + "s");
        live.push("Cheapen level cap (base + gems): " + dep.getMaxCheapenLevel());
    ...
    function renderAscensionHubStatsPillsHtml() {
        if (!dep.getNumber1HasAscended()) return "";
        const totalNodes = dep.getAscensionMapNodeCount();
        const ownedCount = dep.getNumber1AscensionNodeIds().length;
        const invested = dep.getAscensionEssenceInvestedInNodes();
        const pendingBonusEss = dep.getNumber1AscensionPendingBonusEssence();
        const pendingBonusPill = pendingBonusEss > 0
            ? "<span class=\"asc-stat-pill\"><span class=\"asc-stat-pill-k\">Next bonus</span> <span class=\"asc-stat-pill-v\">+" + dep.formatCount(pendingBonusEss) + " Essence</span></span>"
            : "";
        const bhMultStat = dep.getNumber1BlackHoleProductionMult();
        const bhPhase = dep.getBlackHolePhase();
        const p1Spent = Math.floor(Number(dep.getPhase1EssenceSpent()) || 0);
        const bhParallel = Math.max(0, Number(dep.getPhase2ParallelBonusPool()) || 0);
        const blackHolePill = (dep.isBlackHoleArcUnlocked() || bhPhase > 0)
            ? (bhPhase === 1
                ? "<span class=\"asc-stat-pill asc-stat-pill--wide asc-stat-pill--mass\"><span class=\"asc-stat-pill-k\">Mass accumulator</span> <span class=\"asc-stat-pill-v\">charge " + p1Spent + " / " + BLACK_HOLE_PHASE1_ESSENCE_TARGET + " · inertial ×" + dep.formatBlackHolePhase1CpsMultForUi(dep.getBlackHolePhase1RunCpsMult()) + "</span></span>"
                : bhPhase === 7
                    ...
                    : "<span class=\"asc-stat-pill asc-stat-pill--wide asc-stat-pill--void\"><span class=\"asc-stat-pill-k\">Black hole</span> <span class=\"asc-stat-pill-v\">P" + bhPhase + " · ×" + formatBlackHoleMultForUi(bhMultStat) + " · mass " + Math.floor(Number(dep.getPhase2Mass()) || 0) + "</span></span>")
            : "";
        ...
        return (
            ...
            "<span class=\"asc-stat-pill\"><span class=\"asc-stat-pill-k\">Spent in tree</span> <span class=\"asc-stat-pill-v\">" + dep.formatCount(invested) + " Essence</span></span>" +
            "<span class=\"asc-stat-pill\"><span class=\"asc-stat-pill-k\">Pool</span> <span class=\"asc-stat-pill-v\">" + dep.formatCount(dep.getNumber1AscensionEssence()) + "</span></span>" +
            pendingBonusPill +
            blackHolePill +
