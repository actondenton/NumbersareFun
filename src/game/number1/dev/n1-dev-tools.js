...
export function attachN1DevTools(depsRaw) {
    ...
    function devApplyBlackHolePhase() {
        ...
        const beginSuppress = deps.beginDevBhJumpStorySuppression;
        const endSuppress = deps.endDevBhJumpStorySuppression;
        const acknowledgeJumpStories = deps.acknowledgeDevBhJumpStoryBanners;
        const prepareBhState = deps.prepareBlackHoleStateForDevJump;

        if (beginSuppress) beginSuppress();
        try {
            ...
            if (prepareBhState) prepareBhState(phase);
            ...
            if (acknowledgeJumpStories) acknowledgeJumpStories(phase);
