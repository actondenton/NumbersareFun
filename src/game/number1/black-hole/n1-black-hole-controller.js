...
export function createNumber1BlackHoleController(deps) {
    ...
    function tryBuyBlackHolePhase2CollapseUpgrade(track) {
        ...
        deps.addToLog(name + " → tier " + (cur + 1) + "/" + BLACK_HOLE_PHASE2_COLLAPSE_MAX_TIER + parallelNote + ".", "system");
    ...
    function tryBuyNumber1BlackHole() {
        ...
        if (phase === 1) {
            ...
            if (!isBlackHolePhase2MassPourUnlocked()) {
                deps.addToLog(
                    "Stabilize all three collapse upgrades to tier 3 before pouring Essence into mass.",
                    "warning"
                );
                return;
            }
            let L = Math.max(0, Math.min(BLACK_HOLE_PHASE2_MASS_CAP, Math.floor(Number(s.phase2Mass) || 0)));
            if (L >= BLACK_HOLE_PHASE2_MASS_CAP) return;
            const haveEss = Math.max(0, Math.floor(Number(deps.getNumber1AscensionEssence()) || 0));
            if (haveEss < 1) {
                deps.addToLog("Need Ascension Essence to feed mass into the singularity.", "warning");
                return;
            }
            deps.setNumber1AscensionEssence(0);
            let bank = Math.max(0, Math.floor(Number(s.phase2EssenceBank) || 0)) + haveEss;
            ...
            const parallelAdded = addBlackHolePhase2ParallelBonusFromEssence(haveEss);
            const nextC = L < BLACK_HOLE_PHASE2_MASS_CAP ? getBlackHolePhase2CostAtLevel(L) : 0;
            const bankNote =
                bank > 0 && nextC > 0
                    ? (" · " + deps.formatCount(bank) + " / " + deps.formatCount(nextC) + " Essence banked toward the next mass step")
                    : "";
            const parallelNote =
                parallelAdded > 0 ? (" · parallel pool +" + (parallelAdded * 100).toFixed(2) + "%") : "";
            deps.addToLog(
                "Fed " + deps.formatCount(haveEss) + " Essence into black hole mass" + bankNote + parallelNote + ".",
                "system"
            );
